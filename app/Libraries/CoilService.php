...
class CoilService
{
    ...
    private function newDriver()
    {
        if (class_exists('\ModbusMaster'))
            $m = new \ModbusMaster($this->ip, 'TCP');
        elseif (class_exists('\ModbusMasterTcp'))
            $m = new \ModbusMasterTcp($this->ip);
        else
            throw new \RuntimeException('ModbusMaster tidak ditemukan. Cek app/ThirdParty/phpmodbus');

        if (property_exists($m, 'port'))
            $m->port = $this->port;
        if (property_exists($m, 'timeout_sec'))
            $m->timeout_sec = $this->timeout;
        return $m;
    }
